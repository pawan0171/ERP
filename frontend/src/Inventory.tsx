...
export default function Inventory() {
  ...
  const doAdjust = async () => {
    ...
    try {
      ...
      const updateResult = await supabase.from('products').update({ stock_quantity: newStock }).eq('id', p.id);
      if (updateResult.error) {
        console.error('Error updating stock:', updateResult.error);
        setToast(`Error: ${updateResult.error.message}`);
        setTimeout(() => setToast(null), 5000);
        return;
      }
      ...
      if (movementResult.error) {
        ...
        setToast(`Stock updated but movement log failed: ${movementResult.error.message}`);
        setTimeout(() => setToast(null), 5000);
      }
