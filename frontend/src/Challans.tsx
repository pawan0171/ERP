  ...
  const create = async () => {
    ...
    try {
      ...
      if (challanResult.error) {
        console.error('Error creating challan:', challanResult.error);
        setToast(`Error: ${challanResult.error.message}`);
        setSaving(false);
        setTimeout(() => setToast(null), 5000);
        return;
      }

      if (challanResult.data) {
        const itemsResult = await supabase.from('challan_items').insert(
          items.filter((x) => x.product_id).map((x) => ({
            challan_id: challanResult.data.id,
        ...
        if (itemsResult.error) {
          console.error('Error creating challan items:', itemsResult.error);
          setToast(`Challan created but items failed: ${itemsResult.error.message}`);
          setSaving(false);
          setTimeout(() => setToast(null), 5000);
          load();
          return;
      ...
      console.log('Challan created successfully:', challanResult.data);
      setSaving(false);
      setModal(false);
      ...
      console.error('Unexpected error creating challan:', err);
